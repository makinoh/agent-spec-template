...
**本節の限界**: 力量要件は機械検証できません（constitution.md「3. 基本原則」検証手段の選択の人間ゲート（不可避）**(b)** に該当）。本節が機械化するのは「列が空欄のまま出荷されていることの開示」までです。強制台帳への登録は行いません（登録すれば機械強制率の分母だけが増え、実態に反して低下します）。育成・研修の設計は本テンプレートの対象外であり、[SKILLS.md](SKILLS.md) のスキルカタログもこれを代替しません（同ファイル冒頭の注記を参照）。
