...
target_version: null          # constitution.md 0.4.0 → 0.6.0（0.5.0 は並行マージされた WU-05/GP-0006 が使用）／development-process.md 0.2.1 → 0.3.0／standards/ai-governance.md 0.1.0 → 0.2.0／governance/enforcement-ledger.md 0.6.0 → 0.7.0（0.5.0→0.6.0 は WU-05/GP-0006 が使用。いずれも提案・MINOR）
supersedes: []
superseded_by: []
...
| 項目 | 変更 |
| --- | --- |
| AI 生成の識別 | SHOULD → **MUST**。既知の AI エージェント・マシンアカウントが PR 作成者の場合は `ai-generated` ラベルを機械要求（`scripts/checks/pr_governance.sh`）。人間アカウント作成者の場合（AI が支援したが人間が committer である現状の大半のケース）は自己申告に依存する人間ゲート（暫定）とし、強制台帳 #40 で追跡する。 |
| AI 識別トレーラの内容 | 新設。コミットトレーラにモデル識別子・バージョンを含める規定を SHOULD として追加し、**Regulated プロファイル採用時は MUST** とする。強制台帳 #41 で追跡する。 |
...
新規行 #40（AI 生成識別）・#41（トレーラ内容規定）・#42（escape-analysis 記録義務）を追加。当初は #36〜#38 として起案したが、base のマージ時に WU-05（GP-0006）が先に #36〜#39 を採番済みであったため #40〜#42 へ繰り下げた（下記「8.」参照）。凡例直下の注記を、人間ゲート（暫定）行が #36〜#41 の合計6件（WU-05 が4件・本 WU が2件）である旨に更新した。バージョン: 0.6.0（WU-05 到達済み） → **0.7.0（提案・MINOR）**。
...
| WU07-01（人間アカウント作成者） | 同上（自己申告経路） | **人間ゲート（暫定）**（強制台帳 #40） | 機械検証できる作成者側の手がかりが無い（人間の GitHub アカウントから「AI 支援の有無」は判定不能）。専用マシンアカウント発行（#13）後に機械強制へ統合できる見込みがあるため不可避ではなく暫定とした |
| WU07-02 | トレーラのモデル識別子・バージョン記載 | **人間ゲート（暫定）**（強制台帳 #41） | 記載の**形式**検査は将来実装可能（正規表現）だが、**内容の真正性**（自己申告の正確さ）は原理的に機械検証できない。本 WU では形式検査すら実装せず（Regulated プロファイル未採用のため優先度低。範囲外＝スコープ縮小の判断）、将来実装の対象として登録した。「不可避」ではなく「暫定」とした理由: 形式検査自体は技術的に可能であり、「現行技術で不可能」（分類3）には該当しないため |
...
| `task verify:fast` | `scripts/checks/enforcement-ledger.sh` が新規3行（#40〜#42）のスキーマ整合性を検査する。#40・#41 は人間ゲート（暫定）として TBD-HUMAN を記入済みのため合格する |
| コードを含まない採用 | 影響なし |
...
| 文書 | 増分 | 根拠 |
| --- | --- | --- |
| constitution.md | 0.4.0 → 0.6.0（MINOR） | 「7. 定期見直し」への入力追加という後方互換な拡張。既存 SHOULD の反転・削除なし。0.5.0 は base への並行マージで WU-05/GP-0006 が先に使用したため、本エントリは 0.6.0 として登録した（「8.」参照） |
...
| governance/enforcement-ledger.md | 0.6.0 → 0.7.0（MINOR） | 新規行3件（#40〜#42）の追加。既存行の意味変更なし。0.5.0→0.6.0 は WU-05/GP-0006 が先に使用したため、本エントリは 0.7.0 として登録した |
...
`task` 本体が未導入のため、`Taskfile.yml` が定義する各チェックを個別スクリプトとして直接実行した（`verify:fast` ＋ `verify:pr` ＋ `verify` 相当）。**下記は `origin/governance/gp-0003-enforcement-ledger-schema`（WU-05/GP-0006 マージ済み）を本ブランチへ統合した後の最終状態での実行結果**（「8.」参照）。
...
advisory: constitution.md 中の（MUST）/（MUST NOT）出現数 96 件 ／ 台帳行数 43 件（定期見直しで確認する）
✓ Enforcement ledger schema (43 rows checked)
adr/INDEX.md は最新です。
✓ ADR index
...
* WU07-02 のトレーラ**形式**検査（正規表現によるモデル識別子・バージョンらしき文字列の存在確認）は、Regulated プロファイル未採用の現状では実装しなかった。将来 WU の対象とする（強制台帳 #41「移行先ゲート」参照）。
...
本 WU は WU-02（強制台帳のスキーマ拡張）が追加した10列スキーマ（理由区分・失効期限・担当・移行先ゲート）に依存して新規行を追加する。WU-02 の内容が `main` へマージされるまでの間、本 PR は WU-02 のブランチを base として作業する（WU-01・WU-02 と同じ運用）。

**base の再統合（マージ）を実施した経緯**: 本 WU の作業中に、同じ base から並行して起票されていた WU-05（[GP-0006](gp-0006-test-quality-gates.md)、テスト品質ゲート）が `governance/gp-0003-enforcement-ledger-schema` へマージ済みとなり、`governance/enforcement-ledger.md` に行 #36〜#39 が、`constitution.md` にバージョン 0.5.0 の改正履歴エントリが、それぞれ既に存在する状態になった。本 PR が当初 #36〜#38 として採番していた新規行、および constitution.md の 0.5.0 エントリは、この時点で番号・バージョンが重複していた。

これを解消するため、`origin/governance/gp-0003-enforcement-ledger-schema` の最新状態を本ブランチへ `git merge`（`--no-edit`）した。コンフリクトは `constitution.md`（バージョン見出し・改正履歴エントリ）と `governance/enforcement-ledger.md`（新規行・改正履歴エントリ）の2ファイルで発生し、いずれも「WU-05 の内容をそのまま残し、本 WU の内容を後続（より新しいバージョン・より大きい行番号）として追記する」方針で手動解決した。結果として、本 WU の新規行は #40〜#42、`constitution.md`・`governance/enforcement-ledger.md` のバージョンはそれぞれ 0.6.0・0.7.0 に繰り下げた（「5. バージョン増分の判定」参照）。base の付け替え（別ブランチへの rebase）のような、より破壊的な操作は行っていない。
...
| Q-03 | 並行 WU との重複行番号 | **WU-05 との重複は解消済み**（「8.」参照。#36〜#38 → #40〜#42 へ繰り下げ、base をマージ）。ただし WU-03・04・06・08・09 等、本 PR がマージした時点でまだ base に到達していない他の並行作業単位が、同じ #40〜#42 の番号帯を独自に主張する可能性は残る | マージ時に人間が残存する行番号衝突の有無を再確認する |
