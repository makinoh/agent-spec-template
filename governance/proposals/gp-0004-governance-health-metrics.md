...
target_version: "constitution.md: 0.5.0 → 0.6.0（提案・MINOR）／ governance/enforcement-ledger.md: 0.6.0 → 0.7.0（提案・MINOR）"
supersedes: []
superseded_by: []
...
`governance/gp-0003-enforcement-ledger-schema`（WU-02 の作業ブランチ）を土台にしている。`main` を直接の
base にすると WU-01・WU-02 の差分まで本 PR の diff に含まれてしまうため。

**base の途中更新について**: 本 WU の作業中に、同じ base から並行して起票されていた WU-05
（[GP-0006](gp-0006-test-quality-gates.md)。PR #24、マージ済み）が `governance/gp-0003-enforcement-ledger-schema`
へマージされ、強制台帳が35行→39行（#36〜#39 追加）・constitution.md が 0.4.0→0.5.0 へ進んだ。
本提案は起票の過程でこの新しい base へ `git merge` し、コンフリクトを解消した上で自らの新規行を
**#36・#37 から #40・#41 へ改番**した（当初 Q-04 として「衝突しうる」と記載していた懸念が実際に発生し、
本提案の中で解消した）。詳細は「3.5」「4」「7」（Q-04）を参照。
...
| 対象 | 変更 |
| --- | --- |
| 「7. 変更管理」定期見直し | 確認項目に「統治健全性メトリクス（機械強制率の推移、人間ゲート（暫定）残数の推移）」を追加（SHOULD レベル。既存節の voice に合わせた）。算出方法・基準値の正本を governance/enforcement-ledger.md と metrics/governance-health-snapshot.json とし、機械強制率の低下は waiver がない限り機械強制で防ぐ旨を記述（強制台帳 #40・#41 への参照）。 |
| 「13. 改正履歴」 | `[0.6.0]` エントリを追加（本提案の確定を条件とする。WU-05 の `[0.5.0]` エントリの上に積む）。 |
| バージョン | 0.5.0 → **0.6.0（提案。MINOR）**。根拠は本提案「5. バージョン増分の判定」。base merge 前は 0.4.0→0.5.0 として起票していたが、WU-05 が 0.5.0 を既に占有したため 0.6.0 へ改めた。 |

**設計判断（MUST を憲章本文に増設しなかった理由）**: WU03-01/02 が要求する「非減少制約」「waiver 経路」「無条件バイパスの禁止」自体は、
constitution.md 本体に新たな MUST 文として書き下ろさず、強制台帳（8章が定める SSoT）側の新規行（#40・#41）として登録した。
...
* 新規行 #40（機械強制率の非減少制約。MUST・機械強制・整備済み）、#41（waiver 未該当時の無条件バイパス禁止。MUST NOT・機械強制・整備済み）を追加。
* バージョン: 0.6.0 → **0.7.0（提案・MINOR）**。base merge 前は 0.5.0→0.6.0 として起票していたが、WU-05 が 0.6.0 を既に占有したため 0.7.0 へ改めた。

### 2.3 metrics/ — Class A（例外。metrics/README.md 自身が定める昇格規約の適用）

| ファイル | 変更 |
| --- | --- |
| `metrics/governance-health-snapshot.json`（新設） | 機械強制率の baseline スナップショット。強制台帳（本提案が WU-05 の base merge を終えた後の状態。42行）から実測した値をそのまま初期シード値として記録（数値の発明ではなく実測。IMP-05 に整合）。 |
| `metrics/README.md`（変更） | 「位置づけ」に例外節を追加。`metrics/` は本来 Class D・観測専用だが、機械強制率は `task verify:fast` が実際に強制する閾値であるため、README 自身の規約（「ある指標を強制閾値にする場合は standards/ へ移し Class A 化する」）が適用される旨を明記。本 WU では独立の `standards/*.md` は新設せず、`scripts/checks/` への新規チェック追加（それ自体がリポジトリ規約上 Class A）と強制台帳登録（#40・#41）で同じ効果（Class A・機械強制・監査可能）を達成したことを説明する。他の指標（dora.md・ai-metrics.md）は引き続き観測専用で本例外の対象外であることも明記。 |
...
| 方式 | 定義 | 本提案の最終値（42行。WU-05 の #36〜#39 と本 WU の #40・#41 を含む） |
| --- | --- | --- |
| **inclusive（採用）** | 構造的強制または機械強制を**含めば**算入する（人間ゲートが併記されていても減点しない） | 32/42 = 0.7619 |
| exclusive（不採用） | 構造的強制・機械強制の**みで構成される**行（人間ゲートを一切含まない）のみ算入する | 23/42 = 0.5476 |
...
### 3.5 行番号衝突（WU-05）の実際の解消

本提案の起票中に、並行 WU である WU-05（GP-0006／PR #24）が base（`governance/gp-0003-enforcement-ledger-schema`）へ
先にマージされた。WU-05 は強制台帳へ #36〜#39（人間ゲート（暫定）4行）を追加し、constitution.md を 0.4.0→0.5.0、
enforcement-ledger.md を 0.5.0→0.6.0 へ進めていた。本提案は当初 #36・#37 を名乗って起票していたため、
このマージにより実際に行番号の衝突が発生した（起票時点で「4. 影響範囲」に懸念として記載していた事態そのもの）。

解消手順: `git merge origin/governance/gp-0003-enforcement-ledger-schema` を実行し、コンフリクトは
constitution.md・governance/enforcement-ledger.md の2ファイルのみで発生した（いずれも「バージョン見出し」
「改正履歴」「強制台帳の追加行」という、複数 WU が同時に追記すると衝突しやすい箇所であり、他の新設ファイル
（gp-0004 提案書・governance-metrics.py・governance-metrics.sh・metrics/governance-health-snapshot.json 等）は
無衝突だった）。解消方針は「WU-05 の内容を保持したまま、自分の追加分を末尾（#40・#41）へ改番し、バージョンを
WU-05 の到達点からさらに1つ MINOR を進める（constitution.md 0.5.0→0.6.0、enforcement-ledger.md 0.6.0→0.7.0）」
とした。baseline スナップショットの初期シード値も、merge 後の実測値（32/42）に更新した（「3.2」参照）。

この手順・判断自体もAIによる設計選択であり、人間の確認を要する（下記「7. 未解決事項」Q-04）。
...
| `task verify:fast` | 新規チェック（`governance-metrics`）が追加される。現状は baseline と現在値が一致（32/42）のため通過する |
| 既存の強制台帳行（#1〜#39） | 変更なし（本提案による新規2行 #40・#41 の追加のみ。#36〜#39 は WU-05 由来で本提案が base merge を通じて取り込んだもの） |
| `metrics/` の位置づけ | 例外を1件追加（機械強制率のみ）。他の指標（dora.md・ai-metrics.md）は無変更・観測専用のまま |
| コードを含まない採用 | 影響なし（台帳・スクリプトはコードスタック非依存で動作） |
| AI エージェントの権限 | 変更なし |
| 並行して起票されている他 WU（05〜09） | WU-05（GP-0006）との行番号衝突は本提案の起票中に実際に発生し、`git merge` により解消済み（「3.5」参照）。それ以外の並行 WU（04, 06〜09 等）が同じベースから独立に台帳へ行を追加している場合、その WU が後から本ブランチへマージ・rebase する際に同様の衝突・改番が必要になる可能性が残る。本提案はその将来の衝突までは解決しない。 |

## 5. バージョン増分の判定

**constitution.md: 0.5.0 → 0.6.0（提案・MINOR）**
...
* 起票当初は base 側の 0.4.0 の上に 0.4.0→0.5.0 として計画していたが、base merge により WU-05 が
  既に 0.5.0 を使用していたため、その上にさらに一段 MINOR を進めて 0.6.0 とした（「3.5」参照）。

**governance/enforcement-ledger.md: 0.6.0 → 0.7.0（提案・MINOR）**

根拠: 新規行2件（#40・#41）の追加のみで、WU-05 が追加した #36〜#39 を含む既存行の意味を変更していない。
WU-02（0.4.0→0.5.0）・WU-05（0.5.0→0.6.0）と同じ性質（新規行の追加中心）のため MINOR。同じ理由で
起票当初計画の 0.5.0→0.6.0 から 0.6.0→0.7.0 へ改めた。
...
| ID | 内容 | 必要な判断 |
| --- | --- | --- |
| Q-01 | 機械強制率の分子カウント方式に inclusive（構造的強制/機械強制を含めば算入。32/42=0.7619）を採用した。exclusive（人間ゲートを一切含まない行のみ算入。23/42=0.5476）という選択肢も本文「3.1」に併記した | どちらの方式で機械強制率を運用するかの確定。inclusive を採る場合もこの判断自体の妥当性確認 |
...
| Q-04 | 強制台帳の新規行番号は当初 #36・#37 として起票したが、base merge により WU-05（#36〜#39）と実際に衝突し、`git merge` の中で #40・#41 へ改番して解消した（本文「3.5」）。同様の衝突は、他の並行 WU（04, 06〜09 等）が今後同じ base へマージ・rebase する際にも再発しうる | 本提案の改番判断（機械的な末尾追記）を妥当とするか。今後の WU 間の行番号割当調整を誰がいつ行うか（コーディネータが事前採番するか、各 WU が merge 時に都度解消するか） |
...
| 2026-08-20 | claude-code (Sonnet 5) | base（`governance/gp-0003-enforcement-ledger-schema`）を WU-05（GP-0006／PR #24 マージ済み）の到達点へ `git merge`。行番号を #36・#37 から #40・#41 へ改番、バージョン増分を 0.5.0→0.6.0／0.6.0→0.7.0 へ改め、baseline を 32/42 へ再計測 | WU-05 との行番号衝突の解消（「3.5」参照） |
