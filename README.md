...
> **本テンプレートが提供するのは「AIをどう統制するか」（統制層）です。** 仕様化・承認ワークフロー・
> 監査証跡・逸脱の機械検知（`scripts/checks/selftest.sh` 等）に厚く投資しています。
>
> 一方、**「レビューそのものを安く済ませる設計」（アーキテクチャ層）は、ドメイン・技術スタックが
> 決まらないと具体化できないため、本テンプレートは骨格（休眠/活性化ゲート・PRテンプレートの必須欄）
> までしか提供しません**。特にモジュール境界・依存方向（[architecture/boundaries.md](architecture/boundaries.md)）の
> 実体化と、実際の検査ツールの配線（`ARCH_BOUNDARY_CMD` 等）は採用組織の責務です。可逆な設計
> （フィーチャーフラグ・段階公開等）も、PR本文への記載は必須化していますが、実際に可逆な設計を
> 行うかどうかは採用組織の設計判断です。「安全性」＝統治・監査であり、「検証容易性」＝読む量そのものを
> 減らす設計、は別の関心事だと考えてください。詳細は [ADOPTION.md](ADOPTION.md)「ステップ8」を参照してください。
...
*ステータス: 各統治文書は個別にバージョン管理されており（例: constitution.md はファイル冒頭の `* Version:` ヘッダを参照）、現在いずれも `Proposed / ドラフト` 段階です。正式批准（`Accepted`）した文書は `1.0.0` 系列とします（[constitution.md「7. 変更管理」](constitution.md)）。*
