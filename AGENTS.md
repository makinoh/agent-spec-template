...
加えて次を確認し、いずれかに該当する場合は**着手せず質問する**（憲章「6. AIエージェント統治と自律境界」HITL 停止必須チェックポイント。UI 領域の具体化は「10.1.3 推測の禁止」）。
...
`task verify` を実行し、**生の出力を報告に含める**。「問題ありません」だけの報告は無効です（憲章「3. 基本原則」検証は実行可能でなければならない。UI 領域の具体化は「10.1.5」）。
...
基準画像を書き換えられる状態では視覚回帰は検出器として機能しません（憲章「10.1.5」・強制台帳 #27）。
